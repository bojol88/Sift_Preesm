...
void displayRGBInit (int id, int ysize, int xsize)
{
    ...
    if(display.overlays[id] == NULL)
    {
        ...
        display.overlays[id] = SDL_CreateRGBSurface(SDL_SWSURFACE, xsize, ysize,
                               24, 0x0000FF,0x00FF00,0xFF0000,0);
        display.overlays[id]->pitch = 3*xsize;
...
void displayRGB(int id, int ysize, int xsize, unsigned char *rgb){
	unsigned char *r = rgb;
...
void display3Components(int id, unsigned char *r, unsigned char *g, unsigned char *b){
    ...
    for(idxPxl = 0; idxPxl < overlay->h*overlay->w; idxPxl++){
        *(((char*)(overlay->pixels))+3*idxPxl)   = *(r+idxPxl*3) ;
        *(((char*)(overlay->pixels))+3*idxPxl+1) = *(g+idxPxl*3) ;
        *(((char*)(overlay->pixels))+3*idxPxl+2) = *(b+idxPxl*3) ;
    }
