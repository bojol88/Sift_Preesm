...
void
orientation(pointList* keyPointList, pointList* keyPointListCP,
			float scaleSpace[MAX_O*MAX_S*MAX_OCTAVE_SIZE],
            float sigmas[MAX_O*MAX_S], int* octavesW) {
    int i;
	keyPointListCP->size = keyPointList->size;
	for (i = 0; i<keyPointListCP->size; i++) {
		keyPointListCP->list[i] = keyPointList->list[i];
		SiftKeypoint* key = &keyPointListCP->list[i];
        int octave  = key->o;
        float scale = key->s;
